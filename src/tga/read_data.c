...
/*   Updated: 2019/03/22 10:01:41 by lomasse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hdr_parser(t_tga *tga, unsigned char *hdr)
{
	...
	tga->alpha = hdr[17];
	printf("w = %d\nh = %d\nbits_cm = %d\ncolor_type = %d\ncompress = %d\nlen_cm = %d\nbit par pix = %d\nalpha = %d\ncm_begin = %d\n",
			tga->w, tga->h, tga->bits_cm, tga->color_type,
			tga->compress, tga->len_cm,
...
int			read_data(t_tga *tga, int fd)
{
	...
	tga->file = (ft_strcmp((const char *)&tga->file[tga->nb_elem - 18], "TRUEVISION-XFILE.") == 0 ? 
			(unsigned char *)ft_strsub((char const *)tga->file, 0, tga->nb_elem - 26) : tga->file);
	return (1);
}
